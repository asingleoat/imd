...
export class PianoKeyboard {
  ...
  highlightFrequencies(entries, a4 = 440) {
    ...
    const byKey = new Map(); // midi -> { kinds, cents, octaveShifts, minOrder, isTarget }
    for (const { freq, kind, order, target } of entries) {
      ...
      if (!byKey.has(nearestMidi)) {
        byKey.set(nearestMidi, { kinds: new Set(), cents: [], octaveShifts: new Set(), minOrder: Infinity, isTarget: false });
      }
      ...
      if (target) entry.isTarget = true;
    }

    // Apply highlights
    for (const [midi, { kinds, cents, octaveShifts, minOrder, isTarget }] of byKey) {
      const el = this._keys.get(midi);
      el.classList.add('highlighted');
      if (isTarget) el.classList.add('target-match');
  ...
  clearHighlight() {
    for (const el of this._keys.values()) {
      el.classList.remove('highlighted', 'root', 'kind-harmonic', 'kind-sum', 'kind-difference', 'octave-shifted', 'target-match');
      el.style.opacity = '';
