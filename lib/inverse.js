...
// MSE scoring: for each target ratio, find the nearest spectrum frequency
// and compute squared relative error in cents. Missing targets get a fixed
// penalty. Extra frequencies are ignored (they don't hurt — the question
// is whether the desired ratios are present).
export function mseScore(spectrum, targetRatios) {
  const MISSING_PENALTY_CENTS = 600; // a tritone away
  let sumSqErr = 0;
  let matched = 0;
  const matchedKeys = [];
  const missingKeys = [];
  ...
  for (const target of targetRatios) {
    ...
    // Find nearest spectrum frequency
    let bestCents = Infinity;
    for (const f of spectrum) {
      if (f <= 0) continue;
      const cents = 1200 * Math.log2(f / targetValue);
      if (Math.abs(cents) < Math.abs(bestCents)) {
        bestCents = cents;
      }
    }

    if (Math.abs(bestCents) < 50) { // within 50 cents = matched
      matched++;
      matchedKeys.push(key);
      sumSqErr += bestCents * bestCents;
    } else {
      missingKeys.push(key);
      sumSqErr += MISSING_PENALTY_CENTS * MISSING_PENALTY_CENTS;
    }
  }

  const mse = targetRatios.length > 0 ? sumSqErr / targetRatios.length : 0;
  return {
    score: mse,
    matched,
    ...
    matchedKeys,
    missingKeys,
    extraKeys: [], // MSE mode doesn't penalise extras
  };
}
...
export function findInputsForChord(targetIntervals, options = {}) {
  ...
  for (const input of enumerateInputs(mode, maxExtra, { range, dict })) {
    ...
    const barcodeResult = matchScore(barcode, targetRatios, maxDen);
    const mseResult = mseScore(spectrum, targetRatios);

    results.push({
      labels: input.labels,
      ratios: input.ratios,
      barcodeScore: barcodeResult.score,
      mseScore: mseResult.score,
