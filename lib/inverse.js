...
export function mseScore(spectrum, targetRatios) {
  ...
  for (const target of targetRatios) {
    ...
    if (Math.abs(bestCents) < 50) { // within 50 cents = matched
      ...
      // Weight by rank: lower rank = better = lower weighted error
  ...
  const mse = targetRatios.length > 0 ? sumWeightedSqErr / targetRatios.length : 0;
  return {
    score: mse,
    matched,
