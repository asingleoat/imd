...
export function matchScore(barcode, targetRatios) {
  const fingerprint = barcodeFingerprint(barcode);
  const fpKeys = new Set(fingerprint.map(f => f.key));
  ...
  for (const target of targetRatios) {
    ...
    if (fpKeys.has(key)) {
      ...
      const fp = fingerprint.find(f => f.key === key);
      score += fp.birth;
    } else {
