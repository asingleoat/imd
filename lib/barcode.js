...
import { toContinued, convergents } from './fraction.js';
...
function barcodeForRatio(freqRatio, dict, toleranceCents, maxDen) {
  const cf = toContinued(freqRatio);
  const convs = convergents(cf, freqRatio);

  // Walk convergents (skip the formal 1/0 at index 0).
  // Each convergent is the best approximation until the next convergent's
  // denominator is reached. So convergent[i] is active for
  // den[i] <= D < den[i+1].
  const bars = [];
  ...
  for (let i = 1; i < convs.length; i++) {
    ...
    const approxValue = Number(num) / Number(den);
...
export function computeBarcode(freqRatios, dict, toleranceCents = 10, maxDen = 64) {
  const allBars = [];

  for (let idx = 0; idx < freqRatios.length; idx++) {
    const r = freqRatios[idx];
    if (r <= 0) continue;
    const bars = barcodeForRatio(r, dict, toleranceCents, maxDen);
    for (const bar of bars) {
      allBars.push({ ...bar, sourceIndex: idx });
    }
  }

  // Build summary: earliest birth, latest death per ratio
