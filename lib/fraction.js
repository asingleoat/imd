...
// --- Fast Number-only path (no BigInt) for barcode/scoring hot loops ---
// Safe for ratios where numerator and denominator stay within ~2^52.

export function toContinuedFast(x, maxTerms = 30, epsilon = 1e-10) {
  const result = [];
  for (let i = 0; i < maxTerms; i++) {
    const a = Math.floor(x);
    result.push(a);
...
// Returns array of { num, den } (Numbers, not BigInt).
// Skips the formal 1/0 entry.
export function convergentsFast(coeffs) {
  let h2n = 0, h2d = 1; // h_{-2} = 0/1
  let h1n = 1, h1d = 0; // h_{-1} = 1/0
  const result = [];
